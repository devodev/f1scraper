...
use f1_scraper::parse::parse_races_summary;
...
fn query_and_parse(scraper: &Scraper, year: u16) -> Result<format::RaceResultSummary> {
...
fn print(race_summary: &format::RaceResultSummary) -> Result<()> {
    println!("{:?}", race_summary.headers);
    for row in race_summary.summaries.iter() {
        println!("{:?}", row);
    }
    Ok(())
}
